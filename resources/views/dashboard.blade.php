...
@section('title', !empty($isAppliedView) ? 'Applied History — FirstBid AI' : 'Job Inbox — FirstBid AI')

@section('content')
<div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 14px; margin-bottom: 24px;">
  <div>
    <h1 style="font-size: 24px; font-weight: 800; color: var(--text-dark); margin-bottom: 4px;">
      {{ !empty($isAppliedView) ? 'Applied Proposals History' : 'Job Inbox' }}
    </h1>
  ...
  <div style="display: flex; align-items: center; gap: 10px;">
    @if(!empty($isAppliedView))
      <a href="{{ route('dashboard') }}" class="btn btn-ghost btn-sm">← Back to 24h Inbox Feed</a>
    @else
...
<div class="stat-grid">
  <div class="stat-card">
    ...
    <div class="lbl">Jobs Received (24h)</div>
  </div>
  <div class="stat-card">
    <div class="val" style="color: var(--upwork-green);">{{ $stats['letters'] }}</div>
    <div class="lbl">Proposals Written</div>
  </div>
  <a href="{{ route('jobs.applied') }}" class="stat-card" style="text-decoration: none; cursor: pointer; border-color: {{ !empty($isAppliedView) ? 'var(--upwork-green)' : 'var(--border)' }}; background: {{ !empty($isAppliedView) ? 'var(--upwork-tint)' : '#ffffff' }}; transition: all 0.2s ease;">
    <div class="val" style="color: var(--upwork-dark); font-weight: 800;">{{ $stats['applied'] }}</div>
    <div class="lbl" style="color: var(--upwork-tint-text); font-weight: 700;">Applied Proposals ✅</div>
  </a>
  <div class="stat-card">
  ...
  <details class="glass-panel job-card" style="margin-bottom: 0; padding: 0; overflow: hidden;" {{ !empty($isAppliedView) ? 'open' : '' }}>
    ...
    <div style="border-top: 1px solid var(--border); padding: 24px; background: var(--bg-subtle);">
      <div style="font-size: 13.5px; color: var(--text-muted); margin-bottom: 14px; font-weight: 500;">
        ...
        @if($job->is_applied) · <span style="color: var(--upwork-tint-text); font-weight: 700;">🚀 Applied {{ $job->applied_at ? $job->applied_at->diffForHumans() : '' }}</span> @endif
      </div>
...
@empty
  <div class="glass-panel" style="text-align: center; color: var(--text-muted); padding: 50px 20px;">
    @if(!empty($isAppliedView))
      No applied jobs found in history yet. Click <strong>"Mark as Applied 🚀"</strong> on any job proposal when you submit it!
    @else
